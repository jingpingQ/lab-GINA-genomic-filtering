...
dir <- list.dirs()[-c(1,2,3,4,26,27,28)]
dir <- gsub("\\.\\/","",dir)
vaf_cutoff = 7
library(ggplot2)
library(data.table)
...
for (d in 1:length(dir)) {
  ...
  for (i in 1:length(files)) {
   ...
   if(nrow(a[a$diff>0.3 && a$VAF.y<0.2,])>0){
     #check variantion between normal and tumor vaf on same locus
     #if VAF_t-VAF_g>30%, and VAF_g<20%, suspect tumor infiltration
  ...
  #remove germline from list
  snps["germ"] <- NULL
  germ_vaf <- vaf[["germ"]]
  #remove germline vaf from list
  vaf["germ"] <- NULL
...
#   temp_allsnp <- temp_allsnp[!temp_allsnp$gene %in% c("POLE","ATM"),]
  ...
  #create a frame for "somatic snps"
  l <- length(allsnp)-1
  ...
  for (i in 1:length(files)) {
   ...
   extras_vaf <- extras[extras$VAF.x-extras$VAF.y>0.3,]
    ...
    #check if a sample does not have any snp
    ...
    # check if a sample missing any snp
